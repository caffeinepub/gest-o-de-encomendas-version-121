...
import { Plus, X, Loader2, UserPlus, FileImage, Search, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function NewOrcamentoDialog({ open, onOpenChange }: NewOrcamentoDialogProps) {
  ...
  const resetForm = () => {
    ...
    setObservacoes('');
    setCondicoesFornecimento('');
    setValidadeOrcamento('');
    setImagemAdicionalPath(null);
    setIva('23');
    setDesconto('0');
    setItens([{ nomeMaterial: '', descricao: '', quantidade: '', precoUnitario: '' }]);
  };
