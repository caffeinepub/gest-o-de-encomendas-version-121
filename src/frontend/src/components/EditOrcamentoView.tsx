...
import { ArrowLeft, Plus, X, Loader2, FileImage, Search, ChevronDown, ChevronUp } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
import { useOrcamentoQueries, useClienteQueries, useOrderQueries } from '../hooks/useQueries';
import { useFileUpload } from '../blob-storage/FileStorage';
import { toast } from 'sonner';
import type { Orcamento, ItemOrcamento } from '../backend';

interface EditOrcamentoViewProps {
  orcamento: Orcamento;
  onBack: () => void;
}

export default function EditOrcamentoView({ orcamento, onBack }: EditOrcamentoViewProps) {
  ...
  const { useEditOrcamento } = useOrcamentoQueries();
  const { useClientes } = useClienteQueries();
  const { useGestores } = useOrderQueries();
  const editOrcamentoMutation = useEditOrcamento();
    ...
    if (orcamento) {
      ...
      setItens(
        orcamento.itens.map(item => ({
          ...
          precoUnitario: (Number(item.precoUnitario) / 100).toFixed(2)
        }))
      );
    }
  }, [orcamento]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await editOrcamentoMutation.mutateAsync({
        id: orcamento.id,
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 pb-8">
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-2xl font-bold">Editar Orçamento</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" disabled={editOrcamentoMutation.isPending}>
            {editOrcamentoMutation.isPending && (
